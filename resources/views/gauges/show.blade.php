		...
		<div class="panel-container show">
			<div class="panel-content">
				<table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
                    ...
                    <td><a href="{{route('show_contract_devices', 'contract')}}">@if(isset($contract_devices_count)) {{$contract_devices_count}} @endif</a>
						</td>
					</tr>
					<tr>
						<td>Contract Devices Reporting: 
						</td>
						<td><a href="{{route('show_reporting_devices', 'reporting_devices')}}">@if(isset($devices_reporting_count)) {{$devices_reporting_count}} @endif</a>
						</td>
					</tr>
					<tr>
						<td>Devices Not Reporting<span style="color:red;">*</span>:
						</td>
						<td><a href="{{route('show_not_reporting_devices', 'not_reporting')}}">@if(isset($not_reporting_count)) {{$not_reporting_count}} @endif</a>
						</td>
                </tr>
					<tr>
						<td>Devices W/Toner Alert:
						</td>
						<td><a href="{{route('show_toner_data', 'toner')}}">@if(isset($toner_alert_count)) {{$toner_alert_count}} @endif</a>
						</td>
					</tr>
					<tr>
						<td>Devices W/Service Needed:
						</td>
						<td><a href="{{route('show_service_data', 'service_needed')}}">@if(isset($service_needed_count)) {{$service_needed_count}} @endif </a>
						</td>
					</tr>
					<tr>
						<td>Non-Contracted Devices:
						</td>
						<td><a href="{{route('show_non_contract_devices', 'non_contract')}}">@if(isset($non_contract_devices_count)) {{$non_contract_devices_count}} @endif </a>
						</td>
					</tr>
					<tr>
						<td>Last Sync Date:
						</td>
						<td>@if(isset($last_sync_date)) {{$last_sync_date->dt}} @endif</td>
					</tr>
				</table>
				...
				<div class="container-fluid">
					<div class="d-flex my-3">
						...
						<div class="form-group">
							<label for="orgSel">Select an Organization</label>					

							<select name="orgSel" id="orgSel" class="form-control mb-2" onchange="page_direct('gauges.show_by_org', this.value)"> 
				...
				<div class="container-fluid">
					<div class="d-flex my-3">
						<div class="form-group">
							...
							<select name="colSel" id="colSel" class="form-control mb-2" onchange="page_direct('gauges.show_by_color', this.value)">
								<option value="">Select a Color</option>
								<option value="both" @if(isset($color))
                                        				@if($color == 'both')
															selected
														@endif
                                        			@endif>Both</option>
								<option value="black" @if(isset($color))
                                        				@if($color == 'black')
															selected
														@endif
                                        			@endif>Black</option>
								<option value="color" @if(isset($color))
                                        				@if($color == 'color')
															selected
														@endif
                                        			@endif>Color</option>							
				...
				<div class="container-fluid">
					<div class="d-flex my-3">
						<div class="form-group">
							...
							<select name="yearSel" id="yearSel" class="form-control mb-2" onchange="page_direct('gauges.show_by_year', this.value)">
					...
					<div class="panel-content table-responsive">
						<table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
							<tr>
								...
								<td id="consBlk" name="consBlk">@if(isset($cons_black)) {{$cons_black}} @endif
								</td>
							</tr>
							<tr>
								<td>Budgeted:
								</td>
								<td id="bgtBlk">@if(isset($budget_black)) {{$budget_black}} @endif
								</td>
							</tr>
							<tr>
								<td>Projected:
					...
					<div class="panel-content table-responsive">
						<table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
							<tr>
								...
								<td>@if(isset($cons_color)) {{$cons_color}} @endif
								</td>
							</tr>
							<tr>
								<td>Budgeted:
								</td>
								<td>@if(isset($budget_color)) {{$budget_color}} @endif
								</td>
							</tr>
							<tr>
								<td>Projected:
